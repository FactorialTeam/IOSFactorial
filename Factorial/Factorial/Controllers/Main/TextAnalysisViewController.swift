...
class TextAnalysisViewController: UIViewController {
    ...
    @IBAction func tagSchemesSegmentedControlPressed(_ sender: UISegmentedControl) {
        ...
        case 0:
            partOfSpeechSegmentedControl.selectedSegmentIndex = UISegmentedControl.noSegment
    ...
    private func highlightTaggedText(taggedTokens: [TaggedToken]) {
        let attributes = [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18, weight: UIFont.Weight.regular)]
        let attributedString = NSMutableAttributedString(string: textView.text, attributes: attributes)

        let highlightAttributes = [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18, weight: UIFont.Weight.black)]
